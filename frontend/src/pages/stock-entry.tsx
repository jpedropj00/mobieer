import { useQuery } from "@tanstack/react-query";
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { ArrowLeft, Loader2, Package, Trash2 } from "lucide-react";
import { toast } from "sonner";
import { apiGet, apiPost } from "@/services/api";
import type { Product, Supplier } from "@/types";
import { formatNumber, UNITS } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { ProductInputMethod } from "@/features/stock/product-input-method";
import { useAuth } from "@/hooks/use-auth";

type Item = {
  product: Product;
  quantity: string;
  unitValue: string;
  batch: string;
};

export function StockEntryPage() {
  const { can } = useAuth();
  const navigate = useNavigate();
  const [items, setItems] = useState<Item[]>([]);
  const [supplierId, setSupplierId] = useState("");
  const [invoiceNumber, setInvoiceNumber] = useState("");
  const [date, setDate] = useState("");
  const [note, setNote] = useState("");
  const [saving, setSaving] = useState(false);

  const { data: suppliers } = useQuery({
    queryKey: ["suppliers", "all"],
    queryFn: () => apiGet<{ data: Supplier[] }>("/suppliers"),
  });

  if (!can("stock.entry")) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center gap-2 text-center">
        <p className="text-lg font-medium">Sem permissão</p>
        <p className="text-sm text-muted-foreground">Você não tem permissão para registrar entradas.</p>
      </div>
    );
  }

  const updateItem = (index: number, patch: Partial<Item>) => {
    setItems((prev) => prev.map((it, i) => (i === index ? { ...it, ...patch } : it)));
  };

  const addItem = (product: Product) => {
    if (items.some((it) => it.product.id === product.id)) {
      toast.error("Produto já adicionado");
      return;
    }
    setItems((prev) => [...prev, { product, quantity: "1", unitValue: product.unitValue ? String(product.unitValue) : "", batch: "" }]);
  };

  const removeItem = (index: number) => setItems((prev) => prev.filter((_, i) => i !== index));

  const totalValue = items.reduce((acc, it) => {
    const v = Number.parseFloat(it.unitValue.replace(",", "."));
    const q = Number.parseInt(it.quantity, 10);
    if (!Number.isFinite(v) || !Number.isFinite(q)) return acc;
    return acc + v * q;
  }, 0);

  const submit = async () => {
    const parsedItems = items.map((it) => ({
      productId: it.product.id,
      quantity: Number.parseInt(it.quantity, 10),
      unitValue: it.unitValue ? Number.parseFloat(it.unitValue.replace(",", ".")) : null,
      batch: it.batch || null,
    }));
    const invalid = parsedItems.find((i) => !Number.isInteger(i.quantity) || i.quantity <= 0);
    if (invalid) return toast.error("Informe quantidades válidas e maiores que zero");
    if (parsedItems.length === 0) return toast.error("Adicione ao menos um produto");

    setSaving(true);
    try {
      await apiPost("/stock/entries", {
        supplierId: supplierId || null,
        invoiceNumber: invoiceNumber || null,
        date: date ? new Date(date).toISOString() : undefined,
        note: note || null,
        items: parsedItems,
      });
      toast.success("Entrada registrada com sucesso");
      navigate("/movimentacoes");
    } catch (err) {
      toast.error((err as { message?: string }).message ?? "Erro ao registrar entrada");
    } finally {
      setSaving(false);
    }
  };

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="outline" size="icon" onClick={() => navigate(-1)}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-xl font-bold tracking-tight sm:text-2xl">Entrada de estoque</h1>
          <p className="text-sm text-muted-foreground">Registre a chegada de materiais no almoxarifado.</p>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Itens da entrada</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {items.length === 0 && (
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
              Nenhum produto adicionado ainda.
            </div>
          )}
          {items.map((it, i) => (
            <div key={it.product.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center">
              <div className="flex min-w-0 flex-1 items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10">
                  <Package className="h-4 w-4 text-primary" />
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">{it.product.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {it.product.code} · Estoque atual: {formatNumber(it.product.stock)} {UNITS[it.product.unit] ?? it.product.unit}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2 sm:flex-none">
                <div className="flex items-center gap-1">
                  <Input
                    type="number"
                    min={1}
                    value={it.quantity}
                    onChange={(e) => updateItem(i, { quantity: e.target.value })}
                    className="w-24"
                    aria-label="Quantidade"
                  />
                  <span className="text-xs text-muted-foreground">{UNITS[it.product.unit] ?? it.product.unit}</span>
                </div>
                <Input
                  type="text"
                  inputMode="decimal"
                  placeholder="Valor unit."
                  value={it.unitValue}
                  onChange={(e) => updateItem(i, { unitValue: e.target.value })}
                  className="w-28"
                  aria-label="Valor unitário"
                />
                <Input
                  type="text"
                  placeholder="Lote"
                  value={it.batch}
                  onChange={(e) => updateItem(i, { batch: e.target.value })}
                  className="w-28"
                  aria-label="Lote"
                />
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => removeItem(i)}>
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            </div>
          ))}
          <ProductInputMethod onSelect={addItem} excludeIds={items.map((i) => i.product.id)} />
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle>Informações da entrada</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label>Fornecedor</Label>
            <Select value={supplierId} onValueChange={setSupplierId}>
              <SelectTrigger>
                <SelectValue placeholder="Selecione o fornecedor" />
              </SelectTrigger>
              <SelectContent>
                {suppliers?.data.map((s) => (
                  <SelectItem key={s.id} value={s.id}>
                    {s.name}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label>Nota fiscal</Label>
            <Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="Número da NF" />
          </div>
          <div className="space-y-2">
            <Label>Data</Label>
            <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} />
          </div>
          <div className="space-y-2 sm:col-span-2">
            <Label>Observações</Label>
            <Textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Notas adicionais sobre a entrada" />
          </div>
        </CardContent>
      </Card>

      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted-foreground">
          Valor total: <span className="font-semibold text-foreground">{totalValue.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
        </p>
        <Button onClick={submit} disabled={saving || items.length === 0} className="sm:w-auto">
          {saving && <Loader2 className="h-4 w-4 animate-spin" />}
          Registrar entrada
        </Button>
      </div>

    </div>
  );
}
